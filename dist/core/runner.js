    ...
    async verify(testFn, setup) {
        ...
        for (iterations = 1; iterations <= this.maxIterations; iterations++) {
            ...
            try {
                ...
                // Check preconditions
                const preResult = this.contract.checkPreconditions(this.contract.guarantee.preconditions ?? [], ctx);
                if (!preResult.passed) {
                ...
                const result = await this.withTimeout(testFn(ctx), this.timeoutMs, "Test function timed out");
                // Check postconditions
                const postResult = this.contract.checkPostconditions(this.contract.guarantee.postconditions ?? [], ctx, result);
